...
def make_google_llm(model: str):
    """Instancia un ChatGoogleGenerativeAI con el modelo indicado."""
...
with st.sidebar:
    ...
    google_model = st.selectbox("Modelo Google", GOOGLE_MODELS, index=0, disabled=not api_key)
...
with tab_qa:
    ...
    if st.button("Enviar pregunta", type="primary", key="qa_btn"):
        if not question.strip():
            ...
            answer_box = st.empty()
            full_answer = ""
            for token in stream_with_fallback(chain_google, chain_ollama, question, status_slot):
                full_answer += token
...
with tab_summary:
    ...
    if st.button("Generar resumen", type="primary", key="summary_btn"):
        ...
        answer_box = st.empty()
        full_answer = ""
        for token in stream_with_fallback(chain_google, chain_ollama, payload, status_slot):
            full_answer += token
            answer_box.markdown(full_answer)
